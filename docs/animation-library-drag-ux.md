...
- 多目标：弹出确认（主对象 / 全部选中；Group Event 选项灰显「即将支持」）
...
- ~~preferred ids 真正接图层多选~~ **已接**
- ~~多选确认对话框~~ **已接**：`AnimationMultiTargetDropPrompt`（主对象 / 全部；Group 灰显）
...
- [x] 多选确认对话框（PRIMARY / ALL；Group 占位）
- [ ] Group Event 实现
