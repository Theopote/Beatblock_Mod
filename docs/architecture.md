...
落到第 2 层创建路径时（尤其 Animation Library 拖拽）：
...
| 概念 | 代码 | 说明 |
|------|------|------|
| **舞台对象** | `engine.RuntimeStageObject` + `StageObjectSystem` | 一组方块 + 中心点 + 分组规格 |
| **时间轴对象引用** | `timeline.StageObjectRef` | 事件目标 id / 类型 / 显示名（UI 元数据） |
...
生成内容与人工编辑的共存规则（`USER_EDITED`、Build 保护、一次 Undo 的 Smart AutoMap）见 [generation-lifecycle.md](generation-lifecycle.md)。
...
2. **编译冻结（默认播放路径）**：Timeline 仍存 `dispatchModel=STEP` 时，`TimelineCompiler.compileForPlayback` 在编译期用 `StepBurstEventFactory` + `StepSequencePlanner` 展开进 `CompiledTimelineSnapshot`。镜头相关排序（`cameraEdgePriority` / frustum gating / adaptive timing）按**编译期相机轨在 STEP 时间点的采样**冻结，不得读取播放中的 live camera。`PlaybackEngine.load` 与 runtime schedule 拒绝仍含 unresolved STEP 的程序；无引擎的静态 `compile(document)` 可保留 STEP 供检查，但不得用于 Preview / Play / Export。
