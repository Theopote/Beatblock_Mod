 ...
 * <p>
 * Bound clips that are manual / user-edited / locked are never silently deleted.
 */
public final class BuildSequenceCompiler {
	...
	private static boolean compileOne(
		...
		PrepareResult prepare = prepareLayerForBind(timeline, layerManager, layer);
		if (prepare == PrepareResult.PROTECTED) {
			return false;
		}
	...
	/**
	 * If the layer is already bound: replace only replaceable (GENERATED) clips;
	 * leave manual / user-edited / locked choreography alone.
	 */
	private static PrepareResult prepareLayerForBind(
		Timeline timeline,
		BuildLayerManager layerManager,
		BuildLayer layer
	) {
		if (layer.getState() != LayerVisibilityState.BOUND_TO_TRACK) {
			return PrepareResult.NOT_BOUND;
		}
		String boundClipId = layer.getBoundClipId();
		if (boundClipId == null || boundClipId.isBlank()) {
			layerManager.unbindFromClip(layer);
			return PrepareResult.REPLACED;
		}
		for (Track track : BuildLayerTrackSupport.listTracks(timeline)) {
			...
			if (TimelineClipOrigin.isProtectedFromGeneration(clip, track)) {
				return PrepareResult.PROTECTED;
			}
			layerManager.unbindFromClip(layer);
		...
		// Bound id not found on any build track — clear stale bind so compile can continue.
		layerManager.unbindFromClip(layer);
		return PrepareResult.REPLACED;
	}
