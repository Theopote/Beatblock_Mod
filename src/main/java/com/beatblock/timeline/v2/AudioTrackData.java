...
/**
 * 音频轨道专用数据：波形 + 低/中/高频段事件（与现有 WaveformData / FrequencyEvent 一致）。
 */
public class AudioTrackData {
	...
	public List<FrequencyEvent> getLowBand() {
		return Collections.unmodifiableList(lowBand);
	}

	public List<FrequencyEvent> getMidBand() {
		return Collections.unmodifiableList(midBand);
	}

	public List<FrequencyEvent> getHighBand() {
		return Collections.unmodifiableList(highBand);
	}

	public void addFrequencyEvent(com.beatblock.timeline.FrequencyEvent e) {
		if (e == null) return;
		switch (e.getBand()) {
